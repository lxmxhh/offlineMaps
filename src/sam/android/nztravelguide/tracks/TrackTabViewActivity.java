package sam.android.nztravelguide.tracks;
...
import sam.android.nztravelguide.tracks.listview.ExtendedCheckBox;
import sam.android.nztravelguide.tracks.listview.ExtendedCheckBoxListAdapter;
import sam.android.nztravelguide.tracks.listview.IconAdapter;
import sam.android.nztravelguide.tracks.reader.FileImporter;
import sam.android.nztravelguide.tracks.reader.MyParseThread;
import sam.android.nztravelguide.tracks.writer.GpxTrackWriter;
...
//			Log.i("Message", "Tab's id="+tabId);
		...
		if(NZTravelGuide.DBAdapter.checkIfDBHasTracks()) {
			...
			case EDIT_DIALOG: //Dialog id = 0
				...
				builder.setPositiveButton(getString(R.string.OK), new DialogInterface.OnClickListener(){
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						NZTravelGuide.DBAdapter.updateTrack(trackID, changed_track_name, changed_track_des);
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		switch(id)
		{
			...
			case GET_INFORMATION_DIALOG:
				if (!NZTravelGuide.DBAdapter.isMeasureUpdated(trackID)) {
	...
	private void getNameDescriptionFromDB() {
		Cursor myCursor = NZTravelGuide.DBAdapter.getTrack(trackID);
...
//		Log.i("Message", "createBrosweTrackListView");
		...
		Cursor myCursor = NZTravelGuide.DBAdapter.getAllTracks();
	...
	private ArrayList<Location> getLocationListFromDB(long trackID) {
		...
		Cursor myCursor = NZTravelGuide.DBAdapter.getTrackPoints(trackID);
...
//		System.out.println("ResetContentView");
								...
								new Thread(){
									public void run() {
										try {
											...
											NZTravelGuide.addMarkersForDrawing(TrackTabViewActivity.this, placeList, 2);
											finish();
										} catch(Exception e) {
											e.printStackTrace();
										} finally {
	...
	private void deleteSelectedTrackFromDB() {
		if (trackID > 0 ) {	
			try{
				if (NZTravelGuide.DBAdapter.deleteTrack(trackID)) {
