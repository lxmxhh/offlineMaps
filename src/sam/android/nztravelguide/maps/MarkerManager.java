...
import sam.android.nztravelguide.maps.geoutils.Point;
...
public class MarkerManager {
		...
		if(trackType == DrawMarkerForSearch){
			...
			if(NZTravelGuide.isGPSTracking){
				if (markersG.size()>0){
					...
					Location location = NZTravelGuide.currentLocationBeforeRecording;
					if (location != null) {
						double lat = location.getLatitude();
						double lon = location.getLongitude();
						if ((marker.place.getLat() != lat) || (marker.place.getLon() != lon)) {
							markersG.add(marker);
							NZTravelGuide.currentLocationBeforeRecording = null;
						}
					}
				}
			}
			...
			// clear lastTime DB
			if(NZTravelGuide.isDBdrawclear){
				markersDB.clear();
				NZTravelGuide.isDBdrawclear = false;
			}
	...
	public boolean clearSavedTracksG(){
		if(!NZTravelGuide.isGPSTracking){
			savedTrackG.clear();
			NZTravelGuide.isGPSTrackSaved = false;
			return true;
		}else{
			return false;
		}
