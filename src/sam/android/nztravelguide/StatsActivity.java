...
import sam.android.nztravelguide.maps.StatsUtilities;
import sam.android.nztravelguide.tracks.TrackAnalyzer;
...
public class StatsActivity extends Activity implements LocationListener {
	...
	private final Runnable updateResults = new Runnable() {
		public void run() {
			...
			if (NZTravelGuide.isGPSTracking) {
				startTime = NZTravelGuide.recordingTime;
				updateTotalTime();
				updateAllStats();
			}
		}
	...
	private class UIUpdateThread extends Thread {
		...
		@Override
		public void run() {
			while (NZTravelGuide.isGPSTracking && isRunning) {
				long sleeptime = 1000;
	...
	private void registerLocationListener() {
		if (locationManager != null) {
			locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER,
					NZTravelGuide.minTime, NZTravelGuide.minDistance, this);
		}
	}
